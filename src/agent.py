...
def tool_predict_match(team_a: str, team_b: str, host: str | None = None,
    ...
    return {
        ...
        "top_scorers": [
            {"player": s.player, "team": s.team, "probability_pct": round(s.p_scores * 100, 1)}
...
TOOLS = [{
    ...
    "description": ("Statistical EV-optimized prediction for a fixture under the "
                    ...
                    "penalty-free / +2 scorer). Returns a recommended scoreline, "
                    "scoreline alternatives, and the top 3 most likely goalscorers "
                    "(from either team) with their probability of scoring."),
    "input_schema": {
        "type": "object",
        "properties": {
            "team_a": {"type": "string"},
...
because they carry no penalty risk. The scorer pick is independent — report the
top 3 most likely goalscorers from predict_match's top_scorers (player, team,
probability percentage), reordering or excluding any that context rules out.
Finish with: final scoreline, the top 3 most likely scorers with their
probability percentages, expected points, and a 2-3 sentence rationale."""
